...
private:
  ...
  void publish(const NominalState &n)
  {
    ...
    // Caution: NED to ENU
    msg.pose.pose.position.x = n.p.y;
    msg.pose.pose.position.y = n.p.x;
    msg.pose.pose.position.z = -n.p.z;

    msg.twist.twist.linear.x = n.v.y;
    msg.twist.twist.linear.y = n.v.x;
    ...
    tf2::Quaternion q_enu_body = q_enu_ned * q_ned_body;
    q_enu_body.normalize();

    msg.pose.pose.orientation.x = q_enu_body.x();
    msg.pose.pose.orientation.y = q_enu_body.y();
    msg.pose.pose.orientation.z = q_enu_body.z();
    msg.pose.pose.orientation.w = q_enu_body.w();
